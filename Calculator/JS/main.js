...
function Input_Digit(digit) {
    ...
    if(Wait_Second_Operand === true) {
        ...
        //This overwrites Display_Value if value is 0
...
function Input_Decimal(dot) {
    //This ensures that accidrental clicking of decimal point doesn't
...
function Handle_Operator(Next_Operators) {
    ...
    //Check if an operator already exists and if Wait_Second_Operator is true,
    //then updates the operator an exists from the function.
    if (operator && Calculator.Wait_Second_Operand) {
        Calculator.operator = Next_Operators;
        return;
    }
    if (First_Operand == null) {
        ...
        //If an operator exists, properly lookup is performed for the operator
        //in the Perform_Calculation object and the function that matches the
        //operator is executed.
        let result = Perform_Calculation[operator](Value_Now, Value_of_Input);
        //Here we add a fixed amount of numbers after the decimal.
        result = Number(result).toFixed (9);
        //This will remove any trailig 0's
...
function Calculator_Reset() {
    ...
    Calculator.Wait_Second_Operand = false;
    Calculator.operator = nulll;
}
//This function updates the calculator screen with the contents of the Display_Value
